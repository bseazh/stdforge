// 竞争分析管线（业务模块）：可配置查询 → 爬取 → 领域过滤 → hbba 补抓 → 详情补抓 → LLM 提取 → 合并 → 聚合 → LLM 结论
// 供 capability-registry.mjs（模块注册表）与 serve-demo.mjs（演示服务实时接口）复用
import {
  crawlSamrStandards,
...
import { getDomain, DEFAULT_DOMAIN, matchWhitelist } from './domain-config.mjs'
...
// 领域硬过滤：标题含领域词 + （ICS/CCS 白名单命中 或 标题含兜底词）
// 领域配置（标题领域词/兜底词/白名单）统一来自 domain-config.mjs
export const isApplianceFreshness = (standard, domain = DEFAULT_DOMAIN) => {
  const domainConfig = getDomain(domain)
  const title = standard.title || ''
  const ics = standard.ics || ''
  const ccs = standard.ccs || ''
  const titleRe = domainConfig.titlePattern ? new RegExp(domainConfig.titlePattern) : null
  if (titleRe && !titleRe.test(title)) return false
  const icsHit = matchWhitelist(ics, domainConfig.icsWhitelist)
  const ccsHit = matchWhitelist(ccs, domainConfig.ccsWhitelist)
  const fallbackRe = domainConfig.titleFallback ? new RegExp(domainConfig.titleFallback) : null
  return icsHit || ccsHit || (fallbackRe ? fallbackRe.test(title) : false)
}

// 领域过滤原因（调试模式使用）
export const filterReason = (standard, domain = DEFAULT_DOMAIN) => {
  const domainConfig = getDomain(domain)
  const title = standard.title || ''
  const ics = standard.ics || ''
  const ccs = standard.ccs || ''
  const titleRe = domainConfig.titlePattern ? new RegExp(domainConfig.titlePattern) : null
  if (titleRe && !titleRe.test(title)) return `标题不含领域词（${domainConfig.titlePattern}）`
  const icsHit = matchWhitelist(ics, domainConfig.icsWhitelist)
  const ccsHit = matchWhitelist(ccs, domainConfig.ccsWhitelist)
  if (!icsHit && !ccsHit) {
    const fallbackRe = domainConfig.titleFallback ? new RegExp(domainConfig.titleFallback) : null
    if (fallbackRe && !fallbackRe.test(title)) {
      return `ICS/CCS 不在领域白名单（ICS ${domainConfig.icsWhitelist.map((p) => p.source).join('/')}、CCS ${domainConfig.ccsWhitelist.map((p) => p.source).join('/')}）`
    }
  }
  return null
}
...
export const runAnalysis = async ({
  ...
  const safeKeywords = (Array.isArray(keywords) ? keywords : domainConfig.keywords).map((item) => String(item).trim()).filter(Boolean)
  const safeTypes = (Array.isArray(types) ? types : domainConfig.types).filter((type) => ['gb', 'hb', 'db', 'plan'].includes(type))
  ...
  let standards = await timeStep('filter', async () => {
    if (!debugInfo) return crawl.standards.filter((standard) => isApplianceFreshness(standard, domain))
    const kept = []
    for (const standard of crawl.standards) {
      const reason = filterReason(standard, domain)
      if (reason) {
  ...
  return {
    ...
    filteredCount: crawl.standards.filter((standard) => isApplianceFreshness(standard, domain)).length,
